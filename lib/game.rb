...
class Game
  ...
  def make_move
    ...
    if src_piece.valid_move?(move,
                             board)
      (puts "** Piece rules do allow move\n")
    else
      (return rescue_against_piece_move_rules(
        src_piece, move
      ))
    end
    if moves_into_check?(move,
                         active_player)
      (return rescue_against_move_self_into_check(move))
    else
      (puts "** Move doesn't move u into check\n")
    end
  ...
  def add_move(indexed_move)
    raise InputError, "Indexed_move; #{indexed_move} should be formatted like 'iiii'" unless index_format?(indexed_move)

    if true_move?(indexed_move)
      @moves << indexed_move
    else
      (raise InputError,
             "Input; #{indexed_move} does not represent an actual move")
    end
  end
  ...
  def own_piece?(indexed_move)
    x = indexed_move[0].to_i
    y = indexed_move[1].to_i
